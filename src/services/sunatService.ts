import type { SunatValidacion } from "../types";

const BASE_URL =
  "https://ww1.sunat.gob.pe/ol-ti-itconsultaunificadalibre/consultaUnificadaLibre";

const UA =
  "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 Chrome/124.0.0.0 Safari/537.36";

// Mappings tomados del JS de la propia página de SUNAT
const ESTADO_CP: Record<string, string> = {
  "-": "-",
  "0": "NO EXISTE",
  "1": "ACEPTADO",
  "2": "ANULADO",
...
const ESTADO_RUC: Record<string, string> = {
  "-": "-",
  "00": "ACTIVO",
  "01": "BAJA PROVISIONAL",
...
const CONDICION_DOMICILIO: Record<string, string> = {
  "-": "-",
  "00": "HABIDO",
 ...
 */
function generarToken(longitud = 52): string {
...
function toSunatDate(fecha: string): string {
...
export interface SunatConsultaParams {
  numRuc: string;
  codComp: string;
  numeroSerie: string;
  numero: string;
 ...
 */
export async function validarEnSunat(
  params: SunatConsultaParams
): Promise<SunatValidacion> {
  const {
    numRuc,
    codComp,
    numeroSerie,
  ...
  const rawCookies: string[] =
    (getResp.headers as unknown as { getSetCookie?: () => string[] }).getSetCookie?.() ?? [];
  ...
  try {
    ...
    parsed = (typeof outer === "string" ? JSON.parse(outer) : outer) as typeof parsed;
  } catch {
